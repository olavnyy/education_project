...
class Teacher < User
  ...
  scope :students_list, ->(user) { Student.where(group_id: user.group_id, school_id: user.school_id) }
  scope :parents_list, ->(user) { Student.where(group_id: user.group_id, school_id: user.school_id).parents }
